...
uint16_t FLASH_If_CheckAdd(uint32_t SectorAddress);
...
uint16_t FLASH_If_Erase(uint32_t SectorAddress)
{
  ...
  if (SectorAddress < 0x08004000)
  {
    FLASH_EraseSector(FLASH_Sector_0, VoltageRange_3);
  }
  else if (SectorAddress < 0x08008000)
  {
    FLASH_EraseSector(FLASH_Sector_1, VoltageRange_3);
  }
  else if (SectorAddress < 0x0800C000)
  {
  ...
  else if (SectorAddress < 0x08100000)
  {
...
uint8_t *FLASH_If_Read (uint32_t SectorAddress, uint32_t DataLength)
{
  ...
  {
    *(uint32_t*)(MAL_Buffer + idx) = *(uint32_t *)(SectorAddress + idx);
  }
...
uint16_t FLASH_If_CheckAdd(uint32_t SectorAddress)
{
  if ((SectorAddress >= FLASH_START_ADD) && (SectorAddress < FLASH_END_ADD))
  {
    return MAL_OK;
  }
  else

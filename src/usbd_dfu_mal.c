...
static uint8_t  MAL_CheckAdd  (uint32_t SectorAddress);
/* Private functions ---------------------------------------------------------*/
...
{
  uint32_t memIdx = MAL_CheckAdd(SectorAddress);
 
  /* Check if the area is protected */
  if (DFU_MAL_IS_PROTECTED_AREA(SectorAddress))
  {
    return MAL_FAIL;
  }    
  ...
  if (memIdx < MAX_USED_MEDIA)
  {
    ...
    {
      return tMALTab[memIdx]->pMAL_Erase(SectorAddress);
    }
    else
    {
      return MAL_FAIL;
...
uint16_t MAL_Write (uint32_t SectorAddress, uint32_t DataLength)
{
  ...
  if (memIdx < MAX_USED_MEDIA)
  {
    ...
    {
      return tMALTab[memIdx]->pMAL_Write(SectorAddress, DataLength);
    }
    else
    {
      return MAL_FAIL;
...
uint8_t *MAL_Read (uint32_t SectorAddress, uint32_t DataLength)
{
  ...
  if (memIdx < MAX_USED_MEDIA)
  {
    ...
    {
      return tMALTab[memIdx]->pMAL_Read(SectorAddress, DataLength);
    }
    else
    {
      return MAL_Buffer;
...
static uint8_t  MAL_CheckAdd(uint32_t SectorAddress)
{
  ...
  {
    /* If the check addres is positive, exit with the memory index */
    if (tMALTab[memIdx]->pMAL_CheckAdd(SectorAddress) == MAL_OK)
    {
      return memIdx;
    }
  }
